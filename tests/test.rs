...
fn test_03() {
    ...
    let (path, bytes) = squashfs
        .extract_file("Cargo.toml", &dir_blocks, &inodes, &fragments, &root_inode)
        .unwrap();
...
fn test_04() {
    ...
    let (path, bytes) = squashfs
        .extract_file("02", &dir_blocks, &inodes, &fragments, &root_inode)
        .unwrap();
    ...
    let (path, bytes) = squashfs
        .extract_file("03", &dir_blocks, &inodes, &fragments, &root_inode)
        .unwrap();
    ...
    let (path, bytes) = squashfs
        .extract_file("04", &dir_blocks, &inodes, &fragments, &root_inode)
        .unwrap();
    ...
    let (path, bytes) = squashfs
        .extract_file("05", &dir_blocks, &inodes, &fragments, &root_inode)
        .unwrap();
