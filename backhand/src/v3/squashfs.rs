...
use super::compressor::{CompressionOptions, Compressor};
// use super::dir::Dir;
use super::export::Export;
use super::filesystem::node::{InnerNode, Nodes};
...
use super::id::Id;
// use super::metadata;
// use super::reader::{SquashFsReader, SquashfsReaderWithOffset};
// use super::unix_string::OsStringExt;
use crate::kinds::{Kind, LE_V4_0};
// use ::inode::{Inode, InodeId, InodeInner};
...
impl<'b> Squashfs<'b> {
                ...
                for entry in &d.dir_entries {
                    ...
                    let inner: InnerNode<SquashfsFileReader> = match entry.t {
                        ...
                        DirInodeId::BasicFile => {
                            ...
                            InnerNode::File(SquashfsFileReader::Basic(basic))
                        }
                        // Basic Symlink
                        ...
                        DirInodeId::ExtendedSymlink => return Err(BackhandError::UnsupportedInode),
                        DirInodeId::ExtendedBlockDevice => {
                            return Err(BackhandError::UnsupportedInode)
                        }
                        ...
                        DirInodeId::ExtendedSocket => return Err(BackhandError::UnsupportedInode),
                    };
                    let node = Node::new(
                        fullpath.clone(),
                        {
    ...
    pub fn into_filesystem_reader(self) -> Result<FilesystemReader<'b>, BackhandError> {
        ...
        let filesystem = FilesystemReader {
            ...
            no_duplicate_files: false, // Default for v3
        };
        Ok(filesystem)
    }
}
