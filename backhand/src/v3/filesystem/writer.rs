...
use super::super::compressor::{CompressionOptions, Compressor};
use super::super::data::DataWriter;
use super::super::entry::Entry;
use super::super::id::Id;
use super::super::metadata::MetadataWriter;
use super::super::reader::WriteSeek;
use super::super::squashfs::SuperBlock;
use super::super::squashfs::{DEFAULT_BLOCK_SIZE, DEFAULT_PAD_LEN, MAX_BLOCK_SIZE, MIN_BLOCK_SIZE};
use super::node::SquashfsSymlink;
use super::node::{InnerNode, Nodes};
use super::node::{
    Node, NodeHeader, SquashfsBlockDevice, SquashfsCharacterDevice, SquashfsDir, SquashfsFileWriter,
};
...
impl Default for FilesystemWriter<'_, '_, '_> {
    ...
    fn default() -> Self {
        ...
        let block_log = (block_size as f32).log2() as u16;
        Self {
            block_size,
            mod_time: 0,
            id_table: Id::root(),
...
impl<'a, 'b, 'c> FilesystemWriter<'a, 'b, 'c> {
    ...
    pub fn set_time(&mut self, mod_time: u32) {
        self.mod_time = mod_time as i32;
    }

    /// Set time of image as current time
    pub fn set_current_time(&mut self) {
        self.mod_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i32;
    }
    ...
    pub fn from_fs_reader(reader: &'a FilesystemReader<'b>) -> Result<Self, BackhandError> {
        ...
        Ok(Self {
            ...
            mod_time: reader.mod_time as i32,
    ...
    fn write_inode_dir<'slf>(
        ...
        for dir in Entry::into_dir(entries) {
            ...
            dir.to_writer(&mut writer, (kind.inner.type_endian, kind.inner.bit_order.unwrap()))?;
    ...
    pub fn write<W: Write + Seek>(&mut self, mut w: W) -> Result<(SuperBlock, u64), BackhandError> {
        ...
        //     superblock.flags |= Flags::DataHasBeenDeduplicated as u8;
        // }
        ...
        //         .inner
        //         .compressor
        //         .compression_options(self.fs_compressor.id.into(), &self.kind)?;
        //     w.write_all(&options)?;
        // }
