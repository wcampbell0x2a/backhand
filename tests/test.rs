...
fn test_00() {
    ...
    let (path, bytes) = io.extract_file(&squashfs, "squashfs-deku").unwrap();
...
fn test_03() {
    ...
    let (path, bytes) = io.extract_file(&squashfs, "Cargo.toml").unwrap();
...
fn test_05() {
    ...
    let (path, bytes) = io.extract_file(&squashfs, "d").unwrap();
