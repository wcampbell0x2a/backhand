...
use super::filesystem::writer::FilesystemCompressor;
...
use super::filesystem::reader_parallel::SquashfsRawData;
...
impl<'a> DataWriter<'a> {
    ...
    pub(crate) fn just_copy_it<W: WriteSeek>(
        ...
        while let Some(block) = reader.next_block(&mut read_buf) {
            ...
            if block.fragment {
                ...
                let cb = self.kind.compress(
                    &decompress_buf,
                    self.fs_compressor.id.into(),
                    self.block_size,
                )?;
    ...
    pub(crate) fn add_bytes<W: WriteSeek>(
        ...
        while !chunk.is_empty() {
            let cb = self.kind.compress(chunk, self.fs_compressor.id.into(), self.block_size)?;
